...
class AuthModel extends BaseModel
{
    ...
    public function register()
    {
        $data = $this->inputValues();
        $designation = inputPost('designation');
        $rni_number = inputPost('rni_number');
        $mobile = inputPost('mobile');
        // $rni = inputPost('rni');
        $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);
        $data['user_type'] = 'registered';
        $data["slug"] = $this->generateUniqueSlug($data['username']);
        
        if($designation == 'editor' ||  inputPost('designation') == 'unhide'){
            $data['status'] = 0; 
        }else{
            $data['status'] = 1;
        }
        ...
        if ($this->builder->insert($data)) {
            ...
            if ($showAlert) {
                echo '<script>alert("Your account is under review and will be approved by admin.");</script>';
                echo '<script>window.location.href = "'.langBaseUrl().'";</script>';
                exit;
            }
            return true;
        }
